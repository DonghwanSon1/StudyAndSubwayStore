...
public class SubwayOrderController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// checkbox = request.getParameterValues(); => String[] -> 체크된게 1도 없을때 null
		String[] toppings = (String[])request.getParameterValues("topping");
		String[] cookies = (String[])request.getParameterValues("cookie");
		String payment = request.getParameter("payment");
		
		// 3) 가공 => VO클래스 객체 생성해서 거기 감았음
		
		// 4) Service단으로 토스 => 결과값
		
		// 5) 결과에 따른 응답뷰 지정 => 성공 뷰 / 실패 뷰
		// 5_1) 응답뷰로 보낼 수하물 붙이기 => request.setAttribute("키", 벨류);

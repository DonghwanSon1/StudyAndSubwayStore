...
//		}
		...
		// 3단계) 생성된 스트림을 통해서 응답 HTML 구문을 한줄씩 출력
		out.println("<html>");
		out.println("<head>");
		...
		out.printf("<span id = 'height'>%.1f</span>cm이고,  <br>", height);
		
		if(gender == null) {
			out.println("성별을 선택을 안했습니다. <br>");
		}else {
			if(gender.equals("M")) {
				out.println("<span id = 'gender'>성별은 남자</span>입니다.  <br>");				
			}else {
				out.println("<span id = 'gender'>성별은 여자</span>입니다.  <br>");								
			}
		}
		
		if(foods == null) {
			out.println("좋아하는 음식을 선택 안했습니다. <br>");
		}else {
